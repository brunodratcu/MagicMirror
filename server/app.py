...
def get_msal_app():
    ...
    try:
        ...
        if not cfg or not cfg['client_id']:
            print("⚠️ Client ID não configurado")
            return None, None
        
        # IMPORTANTE: Para contas pessoais, usar 'consumers'
        tenant = 'consumers'
        authority = f"https://login.microsoftonline.com/{tenant}"
        
        print(f"🔧 Criando MSAL app:")
        print(f"   Client ID: {cfg['client_id'][:8]}...")
        print(f"   Authority: {authority}")
        
        # Para contas pessoais, sempre PublicClientApplication
        app = msal.PublicClientApplication(
            cfg['client_id'],
            authority=authority
        )
        
        print("✅ MSAL PublicClientApplication criado")
        return app, 'delegated'
        
    except Exception as e:
        print(f"❌ Erro ao criar MSAL app: {e}")
        import traceback
        traceback.print_exc()
        return None, None
...
@app.route('/')
def index():
    for p in ['index.html', './index.html', 'client/index.html']:
        if os.path.exists(p):
            return send_file(p)
...
def save_config():
    ...
    # Para contas pessoais, sempre usar 'consumers'
    conn.execute('''UPDATE config SET 
                    client_id = ?, 
                    tenant_id = ?
                    WHERE id = 1''',
                (cfg['clientId'], 'consumers'))
    conn.commit()
    conn.close()
    
    print(f"✅ Configuração salva - Client ID: {cfg['clientId'][:8]}...")
    
    return jsonify({
        'success': True,
        'mode': 'Delegated Permissions (Conta Pessoal)'
    })
...
def callback():
    ...
    try:
        ...
        print(f"✅ Código OAuth recebido: {code[:20]}...")
        ...
        if "access_token" in result:
            ...
            try:
                ...
                if user_res.status_code == 200:
                    ...
                    print(f"✅ Dados do usuário: {user_name} ({user_email})")
                else:
                    print(f"⚠️ Erro ao obter dados: {user_res.status_code}")
                    user_name = 'Usuário'
                    user_email = 'N/A'
